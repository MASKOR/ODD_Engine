//
// Project: ODD Engine
// Description: Header file for the Database class, which manages domain objects
// and their properties.
//
// This file is part of the ODD Engine and was developed in the Automated
// Driving Project of the Fahrzeugsoftwarelabor at MASCOR Institute of FH Aachen
 ...
 * To suppress the details of the Guardrail, use the boolean parameter
 * hideDetails. It also ensures that the height saved in the Guardlines are
 * correct with the height in the processSequence - map
 */
...
bool ODDEngine::check_logic_block(const LogicBlock &logicBlock) {
  for (const auto &condition : logicBlock.conditions) {
    ...
    if (conditionIsTrue &&
        logicBlock.operationType == LogicBlockType::WHEN_OR) {
      return true;
    }
    if (conditionIsTrue &&
            logicBlock.operationType == LogicBlockType::EXCEPT_WHEN_EITHER ||
        !conditionIsTrue &&
            logicBlock.operationType == LogicBlockType::WHEN_AND) {
      return false;
    }
  }
...
bool ODDEngine::check_guardrail(
    const std::pair<std::string, Guardrail> &guardrailPair,
