...
void ODDEngine::parse_variable(const std::string &key1guardrailName,
  ...
  if (value1guardrailNode.begin()->first.Scalar() == "bool") {
    ...
    throw std::invalid_argument("Type " + value1guardrailNode.begin()->first.Scalar() +
                                " not supported");
  }

  expressionContainer.add_expression(value1guardrailNode.begin()->second.Scalar(),
                                     key1guardrailName, variableTable, dataType);
}
  ...
  for (auto it = value1guardrailNode.begin(); it != value1guardrailNode.end(); ++it) {
    ...
    // Constraints
      ...
      for (auto it = value2NodeInGuardrail.begin(); it != value2NodeInGuardrail.end(); ++it) {
        ...
        if (std::find(KEYWORD_LIST.begin(), KEYWORD_LIST.end(), key3conditionName.Scalar()) !=
            KEYWORD_LIST.end()) {
        ...
        if (key3conditionName.Scalar() == "EXP") {
          ...
          std::string expressionName =
              "_exp" + std::to_string(expression_number) + "_" + key1guardrailName;
          expression_number++;
          expressionContainer.add_expression(expression, expressionName, variableTable,
                                             DataType::BOOL);
          logicBlock.add_bool_condition(expressionName, {"true"},
                                        variableTable.get_bool_ptr(expressionName));
        } else {
      ...
      throw std::invalid_argument(
          "[ERROR] undeclared identifier: " + key2statementInGuardrail.Scalar() +
    ...
    for (auto keywordPair = config.begin(); keywordPair != config.end(); keywordPair++) {
      ...
      if (keywordPair->first.Scalar() == "import") {
        ...
        for (YAML::detail::iterator_value fileIter : keywordPair->second) {
          ...
          if (importFilename.find('/') == std::string::npos) { // filename only contains filename =>
                                                               // file should be in same folder
            importFilepath =
                filename.substr(0, filename.find_last_of("\\/")) + "/" + fileIter.Scalar();
      ...
      // Parse Domain specific Objects
        ...
        for (auto it = value0content.begin(); it != value0content.end(); ++it) {
          ...
          if (it->second.Scalar().empty()) {
            throw std::domain_error("Variable " + it->first.Scalar() +
                                    " doesn't have a domain type");
          }
          if (it->second.Scalar().find('#') == std::string::npos) {
            variableTable.init_object(it->first.Scalar(), dnamespace + it->second.Scalar(),
                                      ontology);
          } else {
